...
class Test_deliberately_cache_a_smart_phrase(unittest.TestCase):
    ...
    def testSpeakOne(self):
        for voice in ('Laura','Sarah'):
            all_oggs = []
            all_mp3s = []
            os.system('rm %s/{*,.*ogg,.*mp3}' % os.path.dirname(pathname_of_phrase_audio(voice,'')))
            os.system('rmdir %s' % os.path.dirname(pathname_of_phrase_audio(voice,'something to say')))
    ...
    def testSpeakTwo(self):
        voice = 'River'
        os.system('rm %s/{*,.*ogg,.*mp3}' % os.path.dirname(pathname_of_phrase_audio(voice,'')))
        os.system('rmdir %s' % os.path.dirname(pathname_of_phrase_audio(voice,'')))
        self.assertRaises(TypeError, lambda : deliberately_cache_a_smart_phrase(voice=voice, smart_phrase=None))
        deliberately_cache_a_smart_phrase(voice=voice, smart_phrase="") # pointless, but acceptable
        deliberately_cache_a_smart_phrase(voice=voice, smart_phrase=" ") #pointless, but acceptable
        ...
        self.assertFalse(os.path.exists(pathname_of_phrase_audio(voice, ' ')))
        oggfiles = list_files_in_dir(os.path.dirname(pathname_of_phrase_audio(voice, '')), endswith_str='ogg')
        mp3files = list_files_in_dir(os.path.dirname(pathname_of_phrase_audio(voice, '')), endswith_str='mp3')
        allfiles = list_files_in_dir(os.path.dirname(pathname_of_phrase_audio(voice, '')))
    ...
    def testJustUsePunctuation(self):
        voice = 'Laura'
        os.system('mkdir "%s"' % os.path.dirname(pathname_of_phrase_audio(voice, '')))
        os.system('rm %s/{*,.*ogg,.*mp3}' % os.path.dirname(pathname_of_phrase_audio(voice,'')))
        self.assertEqual([], list_files_in_dir(os.path.dirname(pathname_of_phrase_audio(voice, ''))))
        deliberately_cache_a_smart_phrase(voice, ' ?')
        deliberately_cache_a_smart_phrase(voice, '!')
        deliberately_cache_a_smart_phrase(voice, '.')
        deliberately_cache_a_smart_phrase(voice, '...')
        deliberately_cache_a_smart_phrase(voice, ' ... ')
    ...
    def testSpeakThreeWithTime(self):
        ...
        os.system('rm %s/{*,.*ogg,.*mp3}' % os.path.dirname(pathname_of_phrase_audio(voice,'')))
        os.system('rmdir %s' % os.path.dirname(pathname_of_phrase_audio(voice,'')))
        self.assertRaises(FileNotFoundError, lambda :list_files_in_dir(os.path.dirname(pathname_of_phrase_audio(voice, ''))))
        os.mkdir(os.path.dirname(pathname_of_phrase_audio(voice,'')))
        allfiles = list_files_in_dir(os.path.dirname(pathname_of_phrase_audio(voice, '')))
...
class Test_generate_detokenized_message(unittest.TestCase):
    ...
    def testMidnightMessage(self):
        ...
        self.assertEqual(["hello,_%s^_it's_12_midnight_and_time's_a-wasting^.mp3" % owner.lower(),
                          ...
                          list_files_in_dir(os.path.dirname(pathname_of_phrase_audio(voice, ''))))
        os.system('rm %s/{*,.*ogg,.*mp3}' % os.path.dirname(pathname_of_phrase_audio(voice,'')))        
    ...
    def testSpecificMessageTen(self):
        ...
        #self.assertRaises(MissingFromCacheError, lambda: smart_phrase_filenames(voice=voice, smart_phrase=my_txt, fail_quietly=False))
        deliberately_cache_a_smart_phrase(voice, my_txt)
        filenames = list_files_in_dir(os.path.dirname(pathname_of_phrase_audio(voice, '')))
        ...
        isactually_basenames = [os.path.basename(r) for r in list_files_in_dir(os.path.dirname(pathname_of_phrase_audio(voice, '')), '.ogg')]
...
#         voice = 'Laura'
#         owner = OWNER_NAME
#         os.system('rm %s/knock*knock*who*there*warrant*by*way*.*' % os.path.dirname(pathname_of_phrase_audio(voice, '')))
