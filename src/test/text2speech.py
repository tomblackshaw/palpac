...
class Test(unittest.TestCase):
    ...
    def testGoofy(self):
        self.assertRaises(ValueError, generate_alarm_message, None, None, None, None, None, None)
        self.assertRaises(ValueError, generate_alarm_message, None, None, None, None, None)
        self.assertRaises(ValueError, generate_alarm_message, None, None, None, None)
        self.assertRaises(ValueError, generate_alarm_message, None, None, None)
        self.assertRaises(ValueError, generate_alarm_message, None, None)
        self.assertRaises(ValueError, generate_alarm_message, None)
        self.assertRaises(ValueError, generate_alarm_message)
        self.assertRaises(ValueError, generate_alarm_message, '')
        ...
        for owner_name in ('Bill', 'Ted', 'Excellent', 'Adventure'):
            for time_24h in range(0, 24):
                for time_minutes in range(0, 60):
                    s = generate_alarm_message(owner_name, time_24h, time_minutes, 'Simple template {shorttime} {owner} yep yep')
                    self.assertRaises(IndexError, owner_name, time_24h, time_minutes, 'Simple {wtf_is_this} template')
